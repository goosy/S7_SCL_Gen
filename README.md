...
- includes    附加代码
  ...
  注意：生成器不检查其中的错误，但会解析文件中用 `(*symbols:` 和 `*)` 包含的注释中的S7符号。

- files    额外复制的自定义代码文件
  类型: 字符串数组
  
  files 只能采用外部文件的方式，指示要在当前功能输出文件中额外复制的SCL或AWL代码文件，在输出文件夹中生成同样名称的文件，注意不会进行编码转换。
  
  其中每一项是以当前配置文件所在的目录为基、`/`符为路径分隔符的相对路径。
  
  默认会将相对路径也复制到目标目录中。如果要改变要复制的相对路径，可用"//"放置在需保留的路径之前。
