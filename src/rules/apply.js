...
async function merge_item(merged_item, item) {
    ...
    if (item.type !== 'convert') { // is not a `convert` item
        return false;
    }
...
export async function apply_action(item, action, parent_tags = null) {
    ...
    if (item !== target) { // an add action or the 1st phase of a merge action
        let error = null;
        if (!parent_tags) {
        ...
        // in add action, continue to execute the following action on target
    }

    // 在 add action 中，$ 必须指向item，其它情况下，item 和 target 相同
...
async function apply_rule(applied_list, list, rule, parent_tags = null) {
    ...
    const item_type = parent_tags ? 'convert' : list[0].type;
    ...
    action_loop: for (const action of actions) {
        ...
        if (action_type === 'delete') {
            ...
            // Only convertion items can be merged
            if (parent_tags || item_type !== 'convert' || matched_items.length === 0) continue;
            const merged_item = {};
            ...
            if (!parent_tags && item_type !== action.type) continue;
