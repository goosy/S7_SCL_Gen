...
export async function apply_action(item, action, using_inner_rules = false) {
    ...
    if (action_type === 'inner_rules') {
        const list = item.tags.list;
        for (const item of list) {
            // save original order
            item.__original_index = (original_index++).toString().padStart(6, '0');
        }
        item.tags.list = await apply_rules(list, action.rules, true);
        return { error: null };
    }
 ...
 * @param {boolean} using_inner_rules Whether the rule is a document rule
 * @returns {Promise<void>}
 */
async function apply_rule(applied_list, list, rule, using_inner_rules) {
    // all items in gen_list are of the same type
    // only convertion items can be merged
    const item_type = list.length > 0 && list[0].type;
    ...
    action_loop: for (const action of actions) {
        ...
        if (action_type === 'delete') {
            ...
            // Only convertion items can be merged
            if (using_inner_rules || item_type !== 'convert' || matched_items.length === 0) continue;
            const merged_item = {};
        ...
        } else if (action_type === 'add') { // add scope
            if (!using_inner_rules && item_type !== action.type) continue;
...
export async function apply_rules(list, rules, using_inner_rules = false) {
    ...
    for (const rule of rules) {
        ...
        else await apply_rule(applied_list, list, rule, using_inner_rules);
    }
    return applied_list;
}
